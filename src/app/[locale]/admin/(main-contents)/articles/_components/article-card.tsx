...
export function ArticleCard({ article }: Props) {
  ...
  return (
    <FlexColumn className="relative justify-between bg-card border rounded-lg shadow-sm">
      <Link href={`/admin/articles/edit/${article.id}`} className="flex flex-col p-6 pb-5 gap-4 h-full hover:bg-muted/50">
        <FlexColumn>
          <ContentStatusWithId id={article.id} publishedAt={article.publishedAt} isArchived={!!article.archivedAt} className="-mt-1 mb-4 -ml-0.5 w-fit" />
        ...
        <AuthorAndDatetime
          ...
          lastEditorName={article.lastEdited.nickname || article.lastEdited.name || ""}
          updatedAt={article.updatedAt}
          createdAt={article.createdAt}
        />
      </Link>

      <ActionBar content={article.atom} articleId={article.id} publishedAt={article.publishedAt} isArchived={!!article.archivedAt} />
    </FlexColumn>
  )
}
...
function TitleAndSummary({
  title, summary, clamp = 1
}: {
  title: string, summary: string, clamp?: 1 | 2
}) {
  return (
    <>
...
function ActionBar({
  ...
  return (
    <FlexRow className="shrink-0 gap-2 border-t px-5 py-1">
      <ArchiveAlertDialog variant={"ghost"} size={"icon"} contentId={articleId} isArchived={isArchived} contentType="article" className="mr-auto" />
