...
import {
  aiArticleRequestSchema,
  aiArticleResponseSchema,
} from "@/types/schema-ai";
...
export const AIAssistant = ({ form, mdxRef }: Props) => {
  ...
  const [prevValues, setPrevValues] =
    useState<z.infer<typeof articleSubmitFormSchema>>();
    ...
    <Popover>
      ...
      >
        <form className="flex flex-col gap-4">
          <Heading>{t("article.aiAssistant.title")}</Heading>
          <AIModelSelector
          ...
          <LanguageSelector
            ...
            onValueChange={(value: Locale) =>
              aiForm.setValue("language", value)
            }
          />
          <Textarea
            disabled={isLoading}
          ...
          <FlexRow gap={3}>
            ...
            {!isLoading && prevValues && (
              <Button
                ...
                size={"icon"}
                variant={"outline"}
                onClick={undone ? handleRedo : handleUndo}
              >
