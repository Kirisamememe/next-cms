    ...
    <>
      <FlexColumn gap={1} className="relative shrink-0">
        <div className="absolute inset-0 -mt-6 -ml-6 z-0 ">
          <GalleryImageItem
            url={imageUrl.url.replace(
              "/upload/",
              "/upload/ar_1.0,c_lfill,h_768/"
            )}
          />
        ...
        <FlexRow gap={4} className="shrink-0 mb-4 z-10">
          <FlexColumn gap={1}>
            ...
            <LabelText>
              {t("gallery.imageUrl.edit.createdAt", {
                datetime: format(imageUrl.createdAt, "PPP p", { locale }),
              })}
            </LabelText>
            <LabelText>
              {t("gallery.imageUrl.edit.updatedAt", {
                datetime: format(imageUrl.createdAt, "PPP p", { locale }),
              })}
            </LabelText>
            <LabelText>
              {t("gallery.imageUrl.edit.author", {
                author:
                  imageUrl.author?.nickname ?? imageUrl.author?.name ?? "",
              })}
            </LabelText>
          </FlexColumn>
        </FlexRow>
