...
import { contentGroupSchema, FormState, ContentListItem, ContentGroupSingleItemForClient, ContentType } from "@/types"
import { zodResolver } from "@hookform/resolvers/zod"
import { createContext, use, useActionState, useCallback, useMemo, useState } from "react"
...
export const GroupFormProvider = ({ children, groupValues }: Props) => {
  ...
  const form = useForm<z.infer<typeof contentGroupSchema>>({
    ...
    defaultValues: {
      ...
      jsonContents: groupValues?.jsonContents.map((item) => Number(item.id)) || [],
      mediaFolders: groupValues?.mediaFolders.map((item) => item.id) || []
    }
  })

  const setListData = useCallback((data: ContentListItem[], type: ContentType) => {
    if (type === 'articles') {
      _setSelectedArticles(data)
      form.setValue(type, data.map((item) => Number(item.id)))
  ...
  const [state, action, pending] = useActionState<FormState>(async () => {
    ...
    const data = groupValues ? await updateContentGroup(groupValues.id, values) : await createContentGroup(values)
    if (!data.isSuccess) {
      toast({
        title: 'Error',
...
export const useGroupFormContext = () => {
  ...
  if (context === undefined) {
    throw new Error('useGroupFormContext must be used within a GroupFormProvider')
  }
