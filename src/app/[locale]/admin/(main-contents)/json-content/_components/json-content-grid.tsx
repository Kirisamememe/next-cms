...
export const JsonContentGrid = async ({
  filter,
  orderby,
  sort,
  searchQuery,
  ...
  const total = (await jsonContentService.getCount(filter, categoryId)) ?? 0;
  const data = await jsonContentService.getMany(filter, {
    take: !searchQuery ? take : undefined,
    orderby,
    sort,
  });
  const filteredByCategory = data.filter(
    (jsonContent) => !categoryId || jsonContent.categoryId === categoryId
  );

  if (!filteredByCategory.length) {
    return <NoContentFound text={t("jsonContent.noContent")} />;
  }
  ...
  const filteredBySearch = filteredByCategory.filter(
    (jsonContent) =>
      ...
      searchArr.some(
        (word) =>
          JSON.stringify(jsonContent.jsonAtom.content)
            .toLowerCase()
            .includes(word.toLowerCase()) ||
          jsonContent.jsonAtom.title
            ?.toLowerCase()
            .includes(word.toLowerCase()) ||
          jsonContent.jsonAtom.description
            ?.toLowerCase()
            .includes(word.toLowerCase())
      )
  );
